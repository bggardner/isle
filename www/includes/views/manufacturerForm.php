...
ob_start();
?>
<form method="post" action="<?= ISLE\Settings::get('web_root'); ?>/manufacturers" class="needs-validation<?= isset($_SESSION['post']) ? ' was-validated' : ''; ?>" novalidate>
...
ISLE\ViewUtility::modalWrapper(
    (isset($_GET['id']) ? 'Edit' : 'Add') . ' Manufacturer',
    ob_get_clean(),
    isset($_GET['id'])
);
?>
