...
if ($_SESSION['user']['role'] >= ISLE\DataModels\Role::CONTRIBUTOR) {
    echo '
    <a href="#" data-form="attributeForm" class="btn btn-primary me-3">
...
if ($count) {
    ...
    <div class="table-responsive">
      <table data-node="attribute" class="table table-striped table-hover align-middle">
        <thead>
          <tr>
            <th scope="col"></th>
        ...
        <tbody>';
    $nodes = ISLE\Service::executeStatement(
        '
  SELECT *
  FROM `' . ISLE\Settings::get('table_prefix') . 'attributes`
  ' . $filter['clause'] . '
  ...
  ' . ISLE\ViewUtility::limitClause() . '
    ...
    while ($row = $nodes->fetch()) {
        ...
        echo '</td>
            ...
            <td>
              <a class="btn btn-sm btn-info" href="' . ISLE\Settings::get('web_root') . '/assets?attribute=' . $row['id'] . '" title="Find Assets">
                <i class="bi-search"></i>
              </a>
            </td>
          </tr>';
    ...
    ISLE\ViewUtility::alert([
        'type' => 'info',
        'text' => 'No' . (count($filter['params']) ? ' matching' : '') . ' attributes found.'
    ]);
    echo '
    </div>';
}
