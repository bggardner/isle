...
class Validate
{
    ...
    static public function stringLength($str, $min_length = 0, $max_length = null)
    {
        if(strlen($str) < $min_length) {
            throw new \Exception('String must be at least ' . $min_length . ' characters long.');
        }
        if (!is_null($max_length) && strlen($str) > $max_length) {
            throw new \Exception('String must be at most ' . $max_length . ' characters long.');
        }
        return $str;
    }
